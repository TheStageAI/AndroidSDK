...
class TTSStreamStats {
  ...
  /// Process memory in MB — the headline "Memory" number. On Android this is
  /// total PSS (proportional set size); on iOS it is `phys_footprint`. Both
  /// are the OS's "how much is this process really using" metric.
  final double? memoryMB;

  /// Resident set size (RSS) in MB — pages currently in physical RAM. Always
  /// <= [memoryMB]; kept as a secondary diagnostic only.
...
// TTSController
// ---------------------------------------------------------------------------
/// Reusable TTS controller for any Flutter app needing streaming
/// text-to-speech.
///
/// Wraps the native TTS pipeline via the thestage_android_sdk plugin.
/// Manages model lifecycle, voice switching, streaming audio playback,
...
class TTSController extends ChangeNotifier {
  ...
  // Latest memory sample (footprint + RSS), refreshed asynchronously during
  // streaming since the native call is async and stats are built
  ...
  Future<void> initialize({String? apiToken}) async {
    ...
    // compile/load stage instead of sitting on "Downloading..." while the
    // backend prepares the model.
  ...
  /// Start streaming TTS for the given text. Audio plays through the
  /// built-in player.
  Future<void> startStream(String text) async {
    ...
    _streamSub = TheStageFlutterSDK.infer_stream(
      ...
      (chunk) {
        // The Android plugin emits audio as a FloatArray, which
        // Flutter's StandardMessageCodec surfaces as a Float32List —
        // exactly what the native AudioStreamPlayer consumes.
        ...
        _stats = TTSStreamStats(
          ...
          // Memory sampled asynchronously via the plugin; values lag by at
          // most one chunk since the native read is async and stats are
          // built synchronously here.
  ...
  /// Sample process memory via the plugin's native reader and cache it. Total
  /// PSS (the headline number) becomes [TTSStreamStats.memoryMB]; RSS is kept
  /// as a secondary diagnostic. Falls back to Dart's `ProcessInfo.currentRss`
  /// (RSS only) if the native call is unavailable.
